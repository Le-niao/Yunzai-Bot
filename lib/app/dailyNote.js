...
export async function dailyNote(e) {
  ...
  let remained_time = "";
  let remained_time_fastest = "";
  if (data.expeditions && data.expeditions.length >= 1) {
    remained_time = lodash.map(data.expeditions, "remained_time");
    remained_time = lodash.max(remained_time);
    remained_time_fastest = lodash.map(data.expeditions, "remained_time");
    remained_time_fastest = lodash.min(remained_time_fastest);
    ...
    if (remained_time_fastest > 0) {
      remained_time_fastest = new Date().getTime() + remained_time_fastest * 1000;
      let remainedDate = new Date(remained_time_fastest);
      remained_time_fastest = format("hh:mm", remainedDate);

      if (format("dd", remainedDate) != nowDay) {
        remained_time_fastest = `明天 ${remained_time_fastest}`;
      } else {
        remained_time_fastest = ` ${remained_time_fastest}`;
      }
    }
  }
